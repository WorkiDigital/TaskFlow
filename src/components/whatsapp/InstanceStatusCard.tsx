...
export function InstanceStatusCard({
  ...
  const getStatusDisplay = (status: WhatsAppConnectionStatus) => {
    switch (status) {
      case "connected":
        return <StatusBadge tone="success">Conectado</StatusBadge>;
      case "waiting_qr":
        return <StatusBadge tone="warning">Aguardando QR Code</StatusBadge>;
      case "connecting":
        return <StatusBadge tone="neutral">Conectando...</StatusBadge>;
      case "error":
        return <StatusBadge tone="danger">Erro</StatusBadge>;
      case "expired":
        return <StatusBadge tone="danger">Expirado</StatusBadge>;
      case "disconnected":
      default:
        return <StatusBadge tone="danger">Desconectado</StatusBadge>;
    }
  };
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Monitore o estado atual da conexão com a Evolution API.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p className="font-medium text-sm">
            {instance.lastSyncAt ? new Date(instance.lastSyncAt).toLocaleString("pt-BR") : "Nunca"}
          </p>
        </div>
      </div>
