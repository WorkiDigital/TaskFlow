...
export function QRCodeConnectionCard({ status, onGenerateQR, isLoading, qrCode, pairingCode }: QRCodeConnectionCardProps) {
  ...
  if (status === 'connected') {
    return (
      <GlassCard className="p-6 flex flex-col items-center justify-center text-center h-full min-h-[300px] border-emerald-500/30 bg-emerald-500/5">
        ...
        <p className="text-sm text-muted-foreground">Sua instância está pronta para enviar e receber mensagens.</p>
      </GlassCard>
    );
  }
    ...
    <GlassCard className="p-6 flex flex-col items-center justify-center text-center h-full min-h-[300px]">
      ...
      {status === 'waiting_qr' ? (
        <div className="flex flex-col items-center">
          <div className="bg-white p-4 rounded-xl mb-4">
            {qrCode?.startsWith('data:image') || qrCode?.startsWith('http') ? (
              <img src={qrCode} alt="QR Code WhatsApp" className="w-48 h-48 object-contain mix-blend-multiply" />
            ) : (
        ...
        <div className="flex flex-col items-center justify-center flex-1 w-full">
          ...
          <p className="text-sm text-muted-foreground mb-6">Gere um QR Code para conectar seu aparelho.</p>
