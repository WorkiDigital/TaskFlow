...
export function WhatsAppCapabilitiesChecklist({ capabilities, connectionStatus }: WhatsAppCapabilitiesChecklistProps) {
  ...
  const getStatusIcon = (status: WhatsAppCapability['status'], depends: boolean) => {
    ...
    switch (status) {
      ...
      case 'not_configured': return <Lock className="w-4 h-4 text-muted-foreground" />;
    }
  };
    ...
    <GlassCard className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {capabilities.map(cap => {
          ...
          const visualStatus = (cap.dependsOnConnection && !isConnected) ? 'not_configured' : cap.status;
          
          return (
            <div key={cap.id} className="flex items-center gap-3 p-3 rounded-lg border border-white/5 bg-white/5">
              <div className="shrink-0 mt-0.5">
                {getStatusIcon(cap.status, cap.dependsOnConnection)}
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium truncate ${visualStatus === 'not_configured' ? 'text-muted-foreground' : 'text-foreground'}`}>
                  {cap.label}
                </p>
