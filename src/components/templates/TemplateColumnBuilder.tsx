...
export function TemplateColumnBuilder({ columns, onChange }: TemplateColumnBuilderProps) {
  ...
  const handleRemoveColumn = (id: string) => {
    const updated = columns.filter(col => col.id !== id).map((col, idx) => ({
      ...col,
      position: idx + 1
    }));
    onChange(updated);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={() => loadPreset('standard')} className="h-7 px-2 text-[11px] gap-1">
            <KanbanSquare className="h-3 w-3" /> Padrão
          </Button>
          <Button variant="outline" size="sm" onClick={() => loadPreset('traffic')} className="h-7 px-2 text-[11px] gap-1">
            <KanbanSquare className="h-3 w-3" /> Tráfego
          </Button>
          <Button variant="outline" size="sm" onClick={() => loadPreset('launch')} className="h-7 px-2 text-[11px] gap-1">
            <KanbanSquare className="h-3 w-3" /> Lançamentos
          </Button>
          <Button size="sm" onClick={handleAddColumn} className="gap-1 bg-primary text-primary-foreground text-xs h-8">
      ...
      <div className="space-y-3">
        {columns.map((column, index) => (
          <GlassCard key={column.id} className="p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border border-border/40 hover:border-border/80 transition-colors">
            ...
            <div className="flex flex-wrap items-center gap-4 w-full md:w-auto justify-end">
                ...
                {PRESET_COLORS.map((preset) => (
                  <button
                    ...
                    className={`h-4.5 w-4.5 rounded-full border transition-all ${preset.class} ${
                      column.color === preset.class ? 'ring-1 ring-primary border-white scale-110' : 'border-transparent hover:scale-105'
                    }`}
                    title={preset.name}
                  />
                ))}
              ...
              <div className="flex items-center gap-2">
                <Switch
                  ...
                  onCheckedChange={(checked) => handleUpdateColumn(column.id, { isFinalColumn: checked })}
                />
                <Label htmlFor={`final-${column.id}`} className="text-xs text-muted-foreground cursor-pointer select-none">
                  Coluna Finalizadora
                </Label>
              </div>
