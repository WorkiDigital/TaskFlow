...
export function TemplateApplySimulation({ templateId, onBack }: TemplateApplySimulationProps) {
  ...
  const steps = [
    { label: "Webhook Recebido", desc: "Processando callback de assinatura eletrônica do Autentique" },
    { label: "Vínculo de Template", desc: "Mapeando título do contrato com template operacional correspondente" },
    { label: "Instanciação do Projeto", desc: "Criando registro do novo projeto no workspace do cliente" },
    ...
    { label: "Atribuição de Responsáveis", desc: "Avançando regras de cargos e fallbacks de usuários" },
    { label: "Notificação e Avisos", desc: "Disparando alertas e e-mails para a equipe de entrega" }
  ];
  ...
  const handleStartSimulation = async () => {
    ...
    // Step 0: Init
    addLog(`[TemplateAutomation] Iniciando simulação de assinatura para cliente "${client.company}"...`, 'info');
    await new Promise(r => setTimeout(r, 600));

    // Step 1: Webhook
    setCurrentStep(0);
    addLog(`[TemplateAutomation] Webhook [Autentique] recebido: contrato assinado por ${client.name}.`, 'success');
    addLog(`[TemplateAutomation] Evento: contract.signed | Contrato: "${template.linkedContractTitle || 'Serviços de Agência'}"`, 'info');
    ...
    if (template.linkedContractTitle) {
      addLog(`[TemplateAutomation] Vínculo correspondente encontrado: "${template.name}" (ID: ${template.id})`, 'success');
    } else {
      addLog(`[TemplateAutomation] Sem vínculo direto de título. Usando seleção manual simulada para "${template.name}".`, 'warn');
    }
    await new Promise(r => setTimeout(r, 800));

    // Step 3: Instanciar Projeto
    setCurrentStep(2);
    addLog(`[TemplateAutomation] Criando projeto "${template.name} — ${client.company}"...`, 'info');
    await new Promise(r => setTimeout(r, 700));
    
    if (simulateError) {
      addLog(`[TemplateAutomation] ERRO: Falha ao inserir registro do projeto na base de dados. Conexão interrompida.`, 'error');
    ...
    addLog(`[TemplateAutomation] Projeto gerado com sucesso. UUID: proj-${Math.floor(Math.random() * 100000)}`, 'success');
    await new Promise(r => setTimeout(r, 700));
    ...
    template.columns.forEach(col => {
      addLog(`[TemplateAutomation]   -> Coluna "${col.title}" criada (Posição ${col.position})`, 'info');
    });
    addLog(`[TemplateAutomation] Total de ${template.columns.length} colunas estruturadas no quadro.`, 'success');
    await new Promise(r => setTimeout(r, 1000));

    // Step 5: Tarefas
    setCurrentStep(4);
    addLog(`[TemplateAutomation] Gerando grade de tarefas (${template.tasks.length} pendentes)...`, 'info');
    let tasksCreated = 0;
    template.tasks.forEach(t => {
      // Calculate date
      const days = t.relativeDueDate.amount;
      const targetDate = format(addDays(new Date(), days), 'dd/MM/yyyy');
      addLog(`[TemplateAutomation]   -> Tarefa "${t.title}" calculada para D+${days} (${targetDate})`, 'info');
      tasksCreated++;
    });
    addLog(`[TemplateAutomation] Total de ${tasksCreated} tarefas agendadas e adicionadas no backlog do projeto.`, 'success');
    await new Promise(r => setTimeout(r, 1000));
    ...
    template.tasks.forEach(t => {
      if (t.assigneeRule.type === 'role') {
        addLog(`[TemplateAutomation]   -> Tarefa "${t.title}": Atribuída ao cargo "${t.assigneeRule.value}". Resolvido para equipe.`, 'info');
      } else if (t.assigneeRule.type === 'specific_user') {
        addLog(`[TemplateAutomation]   -> Tarefa "${t.title}": Atribuída a pessoa fixa "${t.assigneeRule.value}".`, 'info');
      } else {
        addLog(`[TemplateAutomation]   -> Tarefa "${t.title}": Regra "${t.assigneeRule.type}" aplicada.`, 'info');
      }
    });
    ...
    addLog(`[TemplateAutomation] Notificação enviada para WhatsApp interno: "Novo projeto criado: ${template.name} — ${client.company}"`, 'success');
    addLog(`[TemplateAutomation] Integrantes notificados no dashboard.`, 'success');
    await new Promise(r => setTimeout(r, 800));

    // Finish
    setCurrentStep(7); // Completed
    addLog(`[TemplateAutomation] Automação concluída! Projeto "${template.name} — ${client.company}" está ATIVO e pronto para a equipe de entrega.`, 'success');
    setIsRunning(false);
    ...
    try {
      ...
      let spaces = existingSpacesRaw ? JSON.parse(existingSpacesRaw) : [];
      const defaultSpaces = [
      ...
      const newProj = {
        id: newProjId,
        spaceId: template.category === 'launch' ? 'sp-4' : template.category === 'paid_traffic' ? 'sp-2' : 'sp-3',
      ...
      const newTasks = template.tasks.map((t, idx) => {
        const colIndex = template.columns.findIndex(c => c.id === t.columnId);
        const mappedColNum = Math.min(colIndex + 1, 7);
        const mappedColumnId = `col-${mappedColNum}`;
        const mappedStatus = 
          mappedColumnId === 'col-1' ? 'backlog' :
          mappedColumnId === 'col-2' ? 'todo' :
          mappedColumnId === 'col-3' ? 'in_progress' :
          mappedColumnId === 'col-4' ? 'review' :
          mappedColumnId === 'col-5' ? 'waiting' :
          mappedColumnId === 'col-6' ? 'approved' : 'done';
        ...
        return {
          ...
          priority: t.priority,
          checklist: t.checklist.map(c => ({ id: c.id, title: c.title, done: false })),
          comments: [],
          activity: [{ id: `act-task-${idx}`, description: 'Criada via automação de contrato assinado', timestamp: new Date().toISOString() }],
          tags: t.tags
        };
      });
      ...
      console.log('[TemplateApplySimulation] Saved simulated project and tasks to real projects database.');
    } catch (e) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto w-full px-4 py-6 md:px-8 md:py-8">
      ...
      <div className="flex items-center gap-3 border-b border-border/40 pb-4">
        <Button variant="outline" size="icon" onClick={onBack} disabled={isRunning} className="h-8 w-8 text-muted-foreground hover:text-foreground">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          <GlassCard className="p-4 border border-border/40 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                {templates.map(t => (
                  <option key={t.id} value={t.id}>{t.name} ({t.tasks.length} tarefas)</option>
                ))}
              </select>
            </div>
            ...
            <div className="flex items-center justify-between p-2.5 rounded-lg bg-background/40 border border-border/30">
              <div className="flex flex-col">
                ...
                <span className="text-[9px] text-muted-foreground">Simula uma quebra de comunicação de banco.</span>
              </div>
              <Switch
          ...
          <GlassCard className="p-4 border border-border/40 space-y-3.5">
            <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Etapas da Automação</h4>
            <div className="space-y-3.5">
              {steps.map((step, idx) => {
                ...
                return (
                  <div key={idx} className={`flex items-start gap-2.5 text-xs transition-opacity ${isCompleted || isActive || isFailed ? 'opacity-100' : 'opacity-40'}`}>
                    <div className="mt-0.5">
                      <StatusBadge tone={badgeTone}>{statusLabel}</StatusBadge>
                    </div>
                    <div className="space-y-0.5">
                      <span className={`font-semibold ${isActive ? 'text-primary' : isFailed ? 'text-destructive' : 'text-foreground'}`}>
                        {step.label}
                      </span>
          ...
          <GlassCard className="p-0 border border-border/40 bg-black/60 backdrop-blur-2xl rounded-xl flex flex-col flex-1 overflow-hidden h-full">
            ...
            <div className="bg-muted/50 px-4 py-2.5 flex items-center justify-between border-b border-border/40 select-none">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono font-bold tracking-tight text-foreground">CONSOLE LOGS (AUTOMATION_SERVICE)</span>
              </div>
              ...
              <div className="p-4 border-t border-border/40 bg-success/5 flex items-start gap-3">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <p className="text-muted-foreground">
                    O projeto foi integrado no módulo de Gestão de Projetos e já está listado para a agência. 
                    Todas as tarefas foram calculadas com prazos relativos e alocadas conforme as regras.
                  </p>
                </div>
              </div>
            )}
            ...
            {currentStep === -2 && (
              <div className="p-4 border-t border-border/40 bg-destructive/5 flex items-start gap-3">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <p className="text-muted-foreground">
                    A criação foi abortada devido a erro simulado. Verifique os logs do console acima para identificar o gargalo.
                  </p>
                </div>
              </div>
            )}
