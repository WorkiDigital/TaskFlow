...
import {
  Type, AlignLeft, Mail, Phone, CreditCard, Calendar,
  Hash, DollarSign, List, CheckSquare, Upload, Link2,
} from 'lucide-react';

const FIELD_TYPE_CONFIG: Record<FieldType, { label: string; icon: React.ComponentType<{ className?: string }>; inputType?: string }> = {
    ...
    <div className="space-y-5">
      ...
      <div className="pb-4 border-b border-border">
        ...
        <p className="text-xs text-muted-foreground mt-1">
          {form.fields.length} campo{form.fields.length !== 1 ? 's' : ''} · Visualização de como o cliente verá
        </p>
      </div>
      ...
      <div className="space-y-4">
        {form.fields.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-xs text-muted-foreground/60 mt-1">Adicione campos no editor ao lado</p>
          </div>
        ) : (
    ...
    <div className="space-y-1.5">
      ...
      {field.type === 'textarea' ? (
        <div className="min-h-[80px] rounded-xl border border-border bg-input/50 px-3 py-2.5 flex items-start gap-2 cursor-not-allowed">
          ...
          <span className="text-sm text-muted-foreground/50">{field.placeholder || 'Digite aqui...'}</span>
        </div>
        ...
        <div className="h-10 rounded-xl border border-border bg-input/50 px-3 flex items-center justify-between cursor-not-allowed">
          <span className="text-sm text-muted-foreground/50">{field.placeholder || 'Selecione...'}</span>
        ...
        <div className="h-20 rounded-xl border border-dashed border-border bg-input/30 flex flex-col items-center justify-center gap-1 cursor-not-allowed">
          ...
          <span className="text-xs text-muted-foreground/50">{field.placeholder || 'Arraste ou clique para selecionar'}</span>
        </div>
      ) : (
      ...
      {field.helpText && (
        <p className="text-xs text-muted-foreground/70 pl-1">{field.helpText}</p>
      )}
    </div>
  );
}
