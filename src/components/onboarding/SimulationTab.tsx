...
const STATUS_CONFIG: Record<SimLogStatus, { label: string; icon: string; className: string }> = {
  ...
  running: {
    label: "Executando",
    icon: "⚡",
    className: "text-primary bg-primary/10 border-primary/25 animate-pulse",
  },
  done: {
    label: "Concluído",
    icon: "✅",
  ...
  skipped: {
    label: "Pulado",
    icon: "⏭️",
    className: "text-muted-foreground bg-white/5 border-border line-through",
  },
  error: {
    label: "Erro",
    icon: "❌",
...
const STEP_LOGS: Record<string, string[]> = {
  send_contractual_form: [
    "Buscando dados do cliente...",
    "Gerando link de formulário personalizado...",
    "Enviando mensagem via WhatsApp...",
  ],
  await_contractual_form: [
    "Aguardando webhook de confirmação...",
    "Formulário recebido e validado!",
  ],
  generate_contract: [
    "Buscando variáveis preenchidas...",
    "Processando template do contrato...",
    "Documento PDF gerado com sucesso.",
  ],
  send_contract_signature: [
    "Enviando contrato para Autentique...",
    "Coletando assinatura do signatário...",
  ],
  create_whatsapp_group: [
    "Conectando à Evolution API...",
    'Criando grupo: "Projeto {{nome_projeto}}"...',
    "Grupo criado com ID: g.us/1234...",
  ],
  add_participants: [
    "Adicionando gestores ao grupo...",
    "Adicionando cliente ao grupo...",
    "Todos os participantes adicionados.",
  ],
  update_group_description: [
    "Atualizando descrição do grupo...",
    "Descrição definida com sucesso.",
  ],
  send_welcome_message: [
    "Processando mensagem de boas-vindas...",
    "Inserindo variáveis dinâmicas...",
    "Mensagem enviada com formatação WhatsApp.",
  ],
  notify_internal_group: ["Notificando grupo interno da agência...", "Mensagem entregue ao time."],
  send_briefing_form: [
    "Gerando link de briefing...",
    "Enviando mensagem de briefing ao cliente...",
  ],
  await_briefing_form: ["Aguardando retorno do cliente...", "Briefing preenchido e recebido!"],
  finalize_onboarding: [
    "Marcando onboarding como concluído...",
    "Ativando projeto no sistema...",
...
export function SimulationTab({ steps }: SimulationTabProps) {
  ...
  const dispararReal = async () => {
    ...
    try {
      ...
      const lines: string[] =
        res.logs?.map((l: any) => `[${l.status}] ${l.step_name}: ${l.message}`) ?? [];
  ...
  const runSimulation = async () => {
    ...
    setRunning(true);
    console.log(
      "[OnboardingSimulation] starting simulation with",
      activeSteps.length,
      "active steps",
    );
    ...
    for (let i = 0; i < activeSteps.length; i++) {
      ...
      if (shouldError) {
          ...
          prev.map((l) =>
            l.stepId === step.id
              ? {
                  ...l,
                  status: "error",
                  message: `❌ Erro: timeout ao conectar à API externa. Tentando novamente...`,
                }
              : l,
          ),
        );
        ...
        setLogs((prev) =>
          prev.map((l) =>
            l.stepId === step.id
              ? { ...l, status: "done", message: `${step.name} — concluído após retry` }
              : l,
          ),
        );
      } else {
        ...
        setLogs((prev) =>
          prev.map((l) =>
            l.stepId === step.id
              ? { ...l, status: "done", message: `${step.name} — concluído` }
              : l,
          ),
        );
      }
    ...
    <div className="space-y-4">
      ...
      <div className="glass-card p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {activeSteps.length} etapa{activeSteps.length !== 1 ? "s" : ""} ativa
            {activeSteps.length !== 1 ? "s" : ""} · {skippedSteps.length} pulada
            {skippedSteps.length !== 1 ? "s" : ""} · Nenhuma API real será chamada
          </p>
        </div>
          ...
          <div className="p-4 font-mono text-xs space-y-1 max-h-80 overflow-y-auto custom-scrollbar bg-[oklch(0.13_0.015_270)]">
            ...
            {done && (
              <div className="mt-3 pt-3 border-t border-border text-success font-semibold">
                ✅ Simulação concluída em {activeSteps.length} etapas ({skippedSteps.length}{" "}
                puladas)
              </div>
            )}
            <div ref={logEndRef} />
      ...
      <div className="glass-card p-5 space-y-4 border-primary/20">
        <div className="flex items-center gap-2">
          ...
          >
            Chama a edge function de verdade
          </Badge>
        </div>
        <p className="text-xs text-muted-foreground">
          Selecione um cliente e dispare o fluxo real. A função pausará em{" "}
          <code className="text-primary">await_contractual_form</code> e aguardará o preenchimento
          do formulário.
        </p>
