...
export function GroupSelector({ type, onSave, onCancel, initialConfig }: GroupSelectorProps) {
  ...
  const [groupName, setGroupName] = useState(
    initialConfig?.groupName ?? "Projeto {{nome_projeto}}",
  );
  const [participants, setParticipants] = useState(
    Array.isArray(initialConfig?.participants) ? initialConfig.participants.join(", ") : "",
  );
  const [description, setDescription] = useState(
    initialConfig?.description ?? "Grupo oficial de acompanhamento do projeto.",
  );
  ...
  const [internalGroupName, setInternalGroupName] = useState(
    initialConfig?.internalGroupName ?? "",
  );
  const [notifyEvents, setNotifyEvents] = useState<NotifyEvents>(
    initialConfig?.notifyEvents ?? {
              ...
              <div className="space-y-4 pt-4 border-t border-border animate-in fade-in slide-in-from-top-2">
                <div className="space-y-2">
                  <Label className="flex items-center gap-2">
                    <MessageSquare className="w-4 h-4 text-muted-foreground" /> Nome do Grupo
                  </Label>
                  <Input value={groupName} onChange={(e) => setGroupName(e.target.value)} />
                  <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                    <Info className="w-3 h-3 text-primary" /> Voce pode usar variaveis como{" "}
                ...
                <div className="space-y-2">
                  <Label className="flex items-center gap-2">
                    <Phone className="w-4 h-4 text-muted-foreground" /> Adicionar Gestores (Agencia)
                  </Label>
                  <Input
                ...
                <div className="space-y-2">
                  <Label className="flex items-center gap-2">
                    <Settings2 className="w-4 h-4 text-muted-foreground" /> Descricao do Grupo (Bio)
                  </Label>
                  <Textarea
                    value={description}
          ...
          <>
            <div className="glass-card p-5 space-y-6">
              <div className="space-y-2">
                ...
                {groupsError && <p className="text-[10px] text-destructive mt-2">{groupsError}</p>}
            ...
            <div className="glass-card p-5 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between py-2">
                  <Label className="text-sm font-normal text-destructive cursor-pointer">
