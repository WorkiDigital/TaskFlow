...
export function FlowsTab({ steps, onToggle, onUpdate }: FlowsTabProps) {
  ...
  // Returns: 'order' = dependency comes after (true invalid logic)
  //          'disabled' = dependency is off (warning, not broken)
  //          false = no issue
  const checkDependencyError = (step: OnboardingFlowStep): "order" | "disabled" | false => {
    if (!step.enabled || !step.dependsOn || step.dependsOn.length === 0) return false;
    let hasDisabled = false;
    for (const depId of step.dependsOn) {
      const depStep = steps.find((s) => s.id === depId);
      if (!depStep) continue;
      if ((depStep.order ?? 0) >= (step.order ?? 0)) return "order";
      if (!depStep.enabled) hasDisabled = true;
    }
    return hasDisabled ? "disabled" : false;
  };
  ...
  const getStepConfigSummary = (step: OnboardingFlowStep) => {
    if (step.id === "create_whatsapp_group" && Array.isArray(step.config?.participants)) {
      ...
      if (count > 0)
        return `${count} gestor${count === 1 ? "" : "es"} configurado${count === 1 ? "" : "s"}`;
    }

    if (
      step.id === "notify_internal_group" &&
      typeof step.config?.internalGroupName === "string" &&
      step.config.internalGroupName
    ) {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 glass-card p-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <Button
            variant="outline"
            size="sm"
            className="flex-1 sm:flex-none text-xs border-primary/30 hover:bg-primary/10"
            onClick={() => applyTemplateMode("complete")}
          >
            Completo
          </Button>
          <Button
            variant="outline"
            size="sm"
            className="flex-1 sm:flex-none text-xs border-success/30 hover:bg-success/10"
            onClick={() => applyTemplateMode("whatsapp_only")}
          >
            Somente WhatsApp
          </Button>
        </div>
      ...
      <div className="space-y-2">
        {steps
          ...
          .map((step, idx) => {
            ...
            const depError = checkDependencyError(step);
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    step.enabled
                      ? "bg-primary/10 shadow-[0_0_15px_rgba(var(--primary-rgb),0.1)]"
                      : "bg-white/5",
                  )}
                >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {depError === "order" && (
                      <Badge
                        variant="outline"
                        className="text-[10px] px-1.5 bg-destructive/10 text-destructive border-destructive/20 flex items-center gap-1"
                      >
                    ...
                    {depError === "disabled" && (
                      <Badge
                        variant="outline"
                        className="text-[10px] px-1.5 bg-warning/10 text-warning border-warning/20 flex items-center gap-1"
                      >
                        <AlertTriangle className="w-3 h-3" />
                        Dep. desativada
                      </Badge>
                    )}
                    {step.status === "configured" && step.enabled && !depError && (
                      <Badge
                        variant="outline"
                        className="text-[10px] px-1.5 border-success/30 text-success bg-success/5 gap-1"
                      >
                        <Check className="w-2.5 h-2.5" />
                        Configurado
                      </Badge>
                    )}
                    {step.status === "pending" && step.enabled && !depError && (
                      <Badge
                        variant="outline"
                        className="text-[10px] px-1.5 border-warning/30 text-warning bg-warning/5"
                      >
                        Pendente
                      </Badge>
                    )}
                  ...
                  {depError === "order" && (
                    <p className="text-[10px] text-destructive mt-1">
                      Erro de ordem: uma etapa de pré-requisito está posicionada depois desta.
                    </p>
                  )}
                  {depError === "disabled" && (
                    <p className="text-[10px] text-warning mt-1">
                      A etapa &quot;Aguardar formulário&quot; está desativada — ative-a para que os
                      dados do cliente estejam disponíveis ao gerar o contrato.
                    </p>
                  )}
                </div>
