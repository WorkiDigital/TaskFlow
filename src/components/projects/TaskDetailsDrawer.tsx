          ...
          <div className="flex-1 p-6 border-r border-white/5 space-y-6 overflow-y-auto no-scrollbar">
            <SheetHeader className="mb-2">
              <div className="flex items-center gap-2 mb-2">
                ...
                >
                  {mockProjectColumns.find((c) => c.status === formData.status)?.title ||
                    formData.status}
                </Badge>
                {formData.tags?.map((t) => (
                  <Badge
                ...
                <Select
                  ...
                  onValueChange={(v: any) =>
                    setFormData({
                      ...
                      status: v,
                      columnId:
                        mockProjectColumns.find((c) => c.status === v)?.id || formData.columnId,
                    })
                  }
                >
                  <SelectTrigger className="h-8 bg-background/50 border-white/10 text-xs">
              ...
              <div className="flex items-center justify-between">
                <Label>
                  ...
                  {formData.checklist?.length > 0
                    ? `(${Math.round((formData.checklist.filter((c) => c.done).length / formData.checklist.length) * 100)}%)`
                    : ""}
                </Label>
              </div>
