...
import { ChevronDown, ChevronRight, Folder, Hash, Plus, Search, MoreVertical, Edit2, Trash2 } from "lucide-react";
          ...
          return (
            <div key={space.id} className="space-y-1">
              <div 
                ...
                className="w-full flex items-center gap-1.5 px-2 py-1.5 text-xs font-medium text-muted-foreground hover:text-slate-200 rounded-md transition-colors group relative cursor-pointer"
              >
                <div className="w-4 flex items-center justify-center">
                  {isExpanded ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                </div>
                <Folder className={cn("w-3.5 h-3.5 shrink-0", space.color ? space.color.replace('bg-', 'text-') : "text-primary")} />
              ...
              {isExpanded && (
                <div className="pl-6 space-y-0.5">
                  ...
                  {!query && spaceProjects.length === 0 && (
                    <div className="px-2 py-1 text-[10px] text-muted-foreground/50 italic">Vazio</div>
                  )}
                </div>
              )}
            </div>
