...
export function TaskFiltersBar({ 
  ...
  const isAnyFilterActive = searchQuery || statusFilter !== "all" || assigneeFilter !== "all" || sortBy !== "none";
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="h-7 text-[11px] px-2.5 border-white/10 bg-transparent hover:bg-white/5">
              <Filter className="w-3 h-3 mr-1.5" />
              Status: {statusFilter === "all" ? "Todos" : mockProjectColumns.find(c => c.status === statusFilter)?.title || statusFilter}
              <ChevronDown className="w-3 h-3 ml-1.5 opacity-60" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="glass-card">
            <DropdownMenuItem onClick={() => onStatusFilterChange("all")}>Todos os Status</DropdownMenuItem>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="glass-card">
            <DropdownMenuItem onClick={() => onAssigneeFilterChange("all")}>Todos os Responsáveis</DropdownMenuItem>
            {assignees.map(a => (
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="h-7 text-[11px] px-2.5 hover:bg-white/5">
              ...
              Ordenar: {
                sortBy === "none" ? "Nenhum" :
                sortBy === "title" ? "Título" :
                sortBy === "dueDate" ? "Prazo" : "Prioridade"
              }
              <ChevronDown className="w-3 h-3 ml-1.5 opacity-60" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="glass-card">
            <DropdownMenuItem onClick={() => onSortByChange("none")}>Sem ordenação</DropdownMenuItem>
            ...
            <DropdownMenuItem onClick={() => onSortByChange("priority")}>Prioridade</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
