...
export function OperationalAgentPanel({ open, onOpenChange, context }: OperationalAgentPanelProps) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      toast.success(
        `${result.insights.length} insight(s) gerado(s) via ${providerLabel[result.provider]}`,
      );
  ...
  const handleDismissInsight = async (id: string) => {
    try {
      ...
      console.error("[AgentInsights] Erro ao descartar insight:", err);
      const errMsg = err?.message ?? err?.details ?? String(err);
      toast.error(`Erro ao descartar insight: ${errMsg}`);
    }
  };
  ...
  const handleDismissAction = async (id: string) => {
    try {
      ...
      console.error("[AgentInsights] Erro ao descartar ação:", err);
      const errMsg = err?.message ?? err?.details ?? String(err);
      toast.error(`Erro ao descartar ação: ${errMsg}`);
    }
  };
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        >
          ...
          >
            ...
            <TabsContent
              value="insights"
              className="flex-1 overflow-y-auto px-6 pb-6 custom-scrollbar mt-0"
            >
              {insights.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  <Sparkles className="h-8 w-8 text-muted-foreground/40" />
                  <p className="text-sm text-muted-foreground text-center">Nenhum insight ainda.</p>
