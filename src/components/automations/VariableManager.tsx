...
import { Database, Plus, Search, Trash2, Edit2, AlertCircle, Type, User, Briefcase, FileText, Smartphone } from "lucide-react";
...
const defaultVariables: SystemVariable[] = [
  { id: 'v1', key: '{{nome_cliente}}', name: 'Nome do Cliente', source: 'client', mockValue: 'João Silva', inUse: true, isSystem: true },
  { id: 'v2', key: '{{empresa_cliente}}', name: 'Empresa', source: 'client', mockValue: 'Acme Corp', inUse: true, isSystem: true },
  { id: 'v3', key: '{{email_cliente}}', name: 'E-mail do Cliente', source: 'client', mockValue: 'joao@acme.com', inUse: false, isSystem: true },
  { id: 'v4', key: '{{telefone_cliente}}', name: 'Telefone do Cliente', source: 'client', mockValue: '5511999999999', inUse: false, isSystem: true },
  { id: 'v5', key: '{{nome_agencia}}', name: 'Nome da Agência', source: 'agency', mockValue: 'Agência Prime', inUse: true, isSystem: true },
  { id: 'v6', key: '{{nome_projeto}}', name: 'Nome do Projeto', source: 'project', mockValue: 'Lançamento Web 3.0', inUse: true, isSystem: true },
  { id: 'v7', key: '{{valor_projeto}}', name: 'Valor do Projeto', source: 'project', mockValue: 'R$ 15.000,00', inUse: false, isSystem: true },
  { id: 'v8', key: '{{link_formulario_contrato}}', name: 'Link Form Contrato', source: 'form', mockValue: 'https://forms.app/contrato/123', inUse: true, isSystem: true },
  { id: 'v9', key: '{{link_google_drive}}', name: 'Link do Drive', source: 'project', mockValue: 'https://drive.google.com/folders/xyz', inUse: true, isSystem: true },
];
    ...
    <div className="flex flex-col h-full bg-[var(--color-background)] w-full">
      <div className="shrink-0 p-6 border-b border-white/5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Crie e edite as variáveis dinâmicas usadas nas mensagens e formulários.</p>
        </div>
        <Button onClick={startCreate} className="gap-2 bg-primary hover:bg-primary/90 text-primary-foreground">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-2">
            {filteredVars.map(v => (
              <div 
                ...
                onClick={() => { setEditingVar(v); setIsCreating(false); }}
              >
                ...
                <div className="flex items-center justify-between mt-auto pt-2 border-t border-white/5">
                  ...
                  {v.inUse ? (
                    <Badge variant="outline" className="text-[9px] bg-primary/10 text-primary border-transparent h-4 px-1.5">Em Uso</Badge>
                  ) : (
                    <Badge variant="outline" className="text-[9px] bg-white/5 text-muted-foreground border-transparent h-4 px-1.5">Livre</Badge>
                  )}
                </div>
              </div>
            ))}
          ...
          {editingVar ? (
            <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-2">
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground">Chave Técnica (Usada no texto)</Label>
                  <Input 
                  ...
                  {editingVar.isSystem && (
                    <p className="text-[10px] text-yellow-500/80 flex items-center gap-1">
                      <AlertCircle className="w-3 h-3" /> Variáveis do sistema não podem ter nome/chave alterados.
                    </p>
                  )}
                </div>

                <div className="space-y-2">
                  <Label className="text-xs text-muted-foreground">Valor Fictício (Para preview)</Label>
                  <Input 
                  ...
                  />
                  <p className="text-[10px] text-muted-foreground">Este valor será usado apenas para simular a visão das mensagens e contratos no Builder.</p>
                </div>
              ...
              <div className="mt-auto pt-6 flex justify-end gap-3">
                <Button variant="ghost" onClick={() => { setEditingVar(null); setIsCreating(false); }}>Cancelar</Button>
                <Button className="bg-primary hover:bg-primary/90 text-primary-foreground" onClick={handleSaveVariable}>
                  Salvar Alterações
                </Button>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center gap-3">
              ...
              <p className="text-sm text-muted-foreground">Selecione uma variável na lista ou crie uma nova para visualizar suas propriedades.</p>
            </div>
          )}
        </div>
      </div>
