    ...
    <div className="flex flex-col h-full bg-[var(--color-background)] w-full overflow-y-auto">
      <div className="shrink-0 p-6 border-b border-white/5 flex items-center gap-4">
        <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center border border-primary/20 shrink-0">
          {type === 'client' ? <Users className="w-6 h-6 text-primary" /> : <BellRing className="w-6 h-6 text-orange-500" />}
        </div>
        <div>
            ...
            {createAutomatic && (
              <div className="space-y-4 pt-4 border-t border-white/5 animate-in fade-in slide-in-from-top-2">
                <div className="space-y-2">
                  <Label className="flex items-center gap-2"><MessageSquare className="w-4 h-4" /> Nome do Grupo</Label>
                  <Input
                    value={groupName}
                  ...
                  <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                    <Info className="w-3 h-3" /> Voce pode usar a variavel {'{{nome_projeto}}'} no nome.
                  </p>
                </div>

                <div className="space-y-2">
                  <Label className="flex items-center gap-2"><Phone className="w-4 h-4" /> Adicionar Gestores (Agencia)</Label>
                  <Input
                ...
                <div className="space-y-2">
                  <Label className="flex items-center gap-2"><Settings2 className="w-4 h-4" /> Descricao do Grupo (Bio)</Label>
                  <Textarea
                    value={description}
          ...
          <>
            <div className="glass-panel p-5 border border-white/5 rounded-xl space-y-6">
              <div className="space-y-2">
                ...
                {groupsError && (
                  <p className="text-[10px] text-red-400 mt-2">{groupsError}</p>
                )}
            ...
            <div className="glass-panel p-5 border border-white/5 rounded-xl space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between py-2">
                  <Label className="text-sm font-normal text-red-400 cursor-pointer">Erro ou falha tecnica na automacao</Label>
