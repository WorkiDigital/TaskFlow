...
import { Play, CheckCircle2, CircleDashed, AlertCircle, XCircle, ArrowRight, Activity, X } from "lucide-react";
...
export function AutomationSimulationPanel({ flow, onClose }: AutomationSimulationPanelProps) {
    ...
    for (const step of activeSteps) {
      ...
      if (step.dependsOn && step.dependsOn.length > 0) {
        ...
        if (hasMissingDeps) {
          ...
          addLog(`[Erro] A etapa "${step.name}" não pode ser executada porque uma etapa dependente está desativada ou faltando.`, 'error');
      ...
      if (Math.random() < 0.05) {
        setStepStatuses(prev => ({ ...prev, [step.id]: 'error' }));
        addLog(`[Falha] Ocorreu um erro simulado ao tentar executar: ${step.name}. A API retornou timeout.`, 'error');
        setSimulationStatus('error');
  ...
  const getStepIcon = (status: StepStatus | undefined) => {
    switch (status) {
      ...
      case 'skipped': return <CircleDashed className="w-5 h-5 text-muted-foreground opacity-50" />;
      case 'pending': 
      default:
    ...
    <div className="flex flex-col h-full bg-[var(--color-background)] w-full">
      <div className="shrink-0 p-6 border-b border-white/5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Veja a linha do tempo do seu fluxo e teste a execução localmente.</p>
        </div>
      ...
      <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
              ...
              return (
                <div key={step.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  ...
                  <div className={cn(
                    ...
                    status === 'running' ? "bg-blue-500/5 border-blue-500/30 shadow-[0_0_15px_rgba(59,130,246,0.1)]" :
                    status === 'error' ? "bg-red-500/5 border-red-500/30" :
                    "bg-white/5 border-white/10"
                  )}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Etapa {idx + 1}</span>
                      {step.isAutomatic ? (
                        <Badge variant="secondary" className="text-[9px] h-4 bg-primary/10 text-primary hover:bg-primary/20 border-transparent">Automático</Badge>
                      ) : (
                        <Badge variant="secondary" className="text-[9px] h-4 bg-white/10 hover:bg-white/20 border-transparent">Manual</Badge>
                      )}
                    </div>
        ...
        <div className="w-full lg:w-[450px] shrink-0 bg-[#0c0c0e] flex flex-col font-mono text-[13px]">
          <div className="p-4 border-b border-white/5 flex items-center justify-between bg-black/40">
            ...
            <span className="text-muted-foreground text-xs uppercase tracking-wider font-semibold">Simulador de Terminal</span>
          </div>
