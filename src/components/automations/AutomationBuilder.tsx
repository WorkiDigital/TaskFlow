    ...
    const checkWhatsApp = () => {
      ...
      if (saved) {
        try {
          ...
          // ignore parse errors on malformed localStorage value
        }
      }
    };
    checkWhatsApp();
  ...
  const handleSaveMessage = (config: any) => {
    ...
    setFlow((prev) => ({
      ...
      steps: prev.steps.map((s) =>
        s.id === editingStepId ? { ...s, configStatus: "configured" } : s,
      ),
    }));
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--color-background)]">
      <div className="shrink-0 p-6 border-b border-white/5 bg-black/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Configurando o fluxo de execução das tarefas
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 max-w-5xl mx-auto w-full space-y-10">
        <section>
          <AutomationModeSelector selectedMode={flow.mode} onSelectMode={handleModeChange} />
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Arraste para reordenar. Ative apenas o que faz sentido para este fluxo.
              </p>
            </div>
      ...
      <Dialog open={isVarManagerOpen} onOpenChange={setIsVarManagerOpen}>
        <DialogContent className="max-w-5xl h-[80vh] p-0 border-white/10 bg-[var(--color-background)] overflow-hidden flex flex-col">
          ...
          <DialogDescription className="sr-only">
            Crie e edite variáveis dinâmicas do sistema.
          </DialogDescription>
          {isVarManagerOpen && <VariableManager onClose={() => setIsVarManagerOpen(false)} />}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isSimulationOpen} onOpenChange={setIsSimulationOpen}>
        <DialogContent className="max-w-6xl h-[85vh] p-0 border-white/10 bg-[var(--color-background)] overflow-hidden flex flex-col">
          ...
          <DialogDescription className="sr-only">
            Preview visual e simulação em terminal.
          </DialogDescription>
          {isSimulationOpen && (
      ...
      <Dialog open={!!editingStepId} onOpenChange={(open) => !open && setEditingStepId(null)}>
        <DialogContent className="max-w-5xl h-[80vh] p-0 border-white/10 bg-[var(--color-background)] overflow-hidden flex flex-col">
          ...
          <DialogDescription className="sr-only">
            Faça os ajustes necessários nesta etapa da automação.
          </DialogDescription>
