    ...
    switch (step.configStatus) {
      case "configured":
        return (
          <Badge
            variant="outline"
            className="bg-emerald-500/10 text-emerald-500 border-transparent gap-1 text-[10px] uppercase font-bold"
          >
      ...
      case "partial":
        return (
          <Badge
            variant="outline"
            className="bg-yellow-500/10 text-yellow-500 border-transparent gap-1 text-[10px] uppercase font-bold"
          >
      ...
      default:
        return (
          <Badge
            variant="outline"
            className="bg-white/10 text-muted-foreground border-transparent gap-1 text-[10px] uppercase font-bold"
          >
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          {!step.isAutomatic && (
            <Badge variant="secondary" className="text-[9px] h-4 bg-white/10">
              Manual
            </Badge>
          )}
        </div>
        ...
        {isBlockedByWhatsApp && (
          <p className="text-[10px] text-amber-500 mt-1 flex items-center gap-1">
            <AlertCircle className="w-3 h-3" /> Esta etapa depende da conexão WhatsApp. Conecte uma
            instância na aba Configurações {">"} WhatsApp.
          </p>
        )}
      </div>
      ...
      <div className="flex items-center gap-4 shrink-0">
        ...
        <Button
          ...
          className={cn(
            "h-8 gap-1.5",
            (!step.enabled || isBlockedByWhatsApp) && "opacity-50 pointer-events-none",
          )}
        >
