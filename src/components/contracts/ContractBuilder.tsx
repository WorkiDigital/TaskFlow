...
import { ContractTemplate, listContractTemplates, createDraft, getContractTemplate, sendContractToAutentique, updateContract } from "@/services/contractsService";
...
export function ContractBuilder() {
  ...
  useEffect(() => {
    ...
    getContractTemplate(selectedTemplateId)
      .then((res) => {
        if (res.error) toast.error(res.error);
        else if (res.data) setForm((prev) => ({ ...prev, clauses: res.data!.content ?? prev.clauses }));
      })
        ...
        <div className="mt-5 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input id="ct-signer-name" placeholder="Nome completo" value={form.signerName} onChange={(e) => setForm({ ...form, signerName: e.target.value })} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="ct-signer-email">E-mail do signatário</Label>
              <Input id="ct-signer-email" type="email" placeholder="email@cliente.com" value={form.signerEmail} onChange={(e) => setForm({ ...form, signerEmail: e.target.value })} />
            </div>
          </div>
          ...
          <div className="flex justify-end gap-2">
            <Button variant="ghost" onClick={() => setPreviewOpen(true)}>Pré-visualizar</Button>
            <Button onClick={save} disabled={loadingTemplates}>Salvar rascunho</Button>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto bg-background border-border">
          ...
          <div className="prose prose-sm dark:prose-invert max-w-none space-y-4 py-2">
            <div className="border-b border-border pb-4 space-y-1">
              ...
              {form.client && <p className="text-sm text-muted-foreground m-0">Cliente: <span className="font-medium text-foreground">{form.client}</span></p>}
              {form.value && <p className="text-sm text-muted-foreground m-0">Valor: <span className="font-medium text-foreground">R$ {form.value}</span></p>}
              {form.signerName && <p className="text-sm text-muted-foreground m-0">Signatário: <span className="font-medium text-foreground">{form.signerName}</span></p>}
              {form.signerEmail && <p className="text-sm text-muted-foreground m-0">E-mail: <span className="font-medium text-foreground">{form.signerEmail}</span></p>}
            </div>
            <div
