...
const PERMISSION_KEYS = [
  {
    ...
    label: "Configurações da Agência",
    desc: "Permite alterar nome, logo e dados gerais",
  },
  {
    key: "manage_integrations",
    label: "Integrações",
    desc: "Configurar APIs (Autentique, WhatsApp, etc)",
  },
  {
    key: "manage_team",
    label: "Gerenciar Equipe",
    desc: "Convidar, editar e suspender membros da equipe",
  },
  { key: "manage_clients", label: "Gerenciar Clientes", desc: "Criar, editar e excluir clientes" },
  {
    ...
    desc: "Criar e enviar contratos de serviços",
  },
  {
    key: "manage_onboarding",
    label: "Gerenciar Onboarding",
    desc: "Configurar e acompanhar etapas de onboarding",
  },
  {
    key: "manage_automations",
    label: "Gerenciar Automações",
    desc: "Criar e editar fluxos e gatilhos automatizados",
  },
  {
    key: "manage_templates",
    label: "Gerenciar Templates",
    desc: "Gerenciar templates de projetos e tarefas",
  },
  {
    key: "manage_projects",
    label: "Gerenciar Projetos",
    desc: "Criar projetos, colunas e gerenciar kanban geral",
  },
  {
    key: "create_tasks",
    label: "Criar Tarefas",
    desc: "Criar novas tarefas nos projetos em que atua",
  },
  { key: "assign_tasks", label: "Atribuir Tarefas", desc: "Definir responsáveis para as tarefas" },
  {
    key: "approve_agent_actions",
    label: "Aprovar Ações do Agente",
    desc: "Aprovar sugestões de ações do Agente Operacional",
  },
  {
    key: "execute_agent_actions",
    label: "Executar Ações do Agente",
    desc: "Permitir que o agente execute tarefas automaticamente",
  },
  {
    key: "view_all_projects",
    label: "Ver Todos os Projetos",
    desc: "Ver projetos mesmo sem estar atribuído",
  },
  {
    key: "view_dashboard",
    label: "Visualizar Dashboard",
    desc: "Acesso aos gráficos e métricas de desempenho",
  },
];
...
export function AgencyRolesModal({ open, onOpenChange }: AgencyRolesModalProps) {
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Tem certeza que deseja excluir este cargo? Os usuários vinculados ficarão sem cargo customizado.",
      )
    )
      return;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-background border-border sm:max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Crie cargos customizados (ex: Gestor de Tráfego, Designer) e defina quais permissões
            cada um possui na plataforma.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {isLoading && roles.length === 0 ? (
                ...
                <p className="text-sm text-muted-foreground italic bg-secondary/10 p-4 rounded-lg text-center">
            ...
            <div className="border-b border-border/40 pb-3 mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">
                  {isEditing
                    ? `Defina o que "${newRoleName}" pode fazer:`
                    : "Selecione um cargo para gerenciar permissões."}
                </p>
              </div>
            </div>
                  ...
                  <div key={p.key} className="flex items-start justify-between gap-3 text-xs">
                    <div className="space-y-0.5">
                      <Label
                        ...
                        onClick={() => togglePermission(p.key, !isEnabled)}
                      >
                        {p.label}
                      </Label>
