...
function withDefaults(state: Partial<OnboardingWorkspaceState>): OnboardingWorkspaceState {
  return {
    flowSteps: state.flowSteps?.length ? state.flowSteps : initialOnboardingState.flowSteps,
    formTemplates: state.formTemplates?.length ? state.formTemplates : initialOnboardingState.formTemplates,
  ...
  const applyTemplateMode = useCallback((mode: 'complete' | 'whatsapp_only' | 'custom') => {
    setState(prev => {
      ...
      if (mode === 'complete') {
        ...
        const waSteps = ['create_whatsapp_group', 'add_participants', 'update_group_description', 'send_welcome_message', 'notify_internal_group', 'finalize_onboarding'];
  ...
  const saveFormTemplate = useCallback((form: FormTemplate) => {
    setState(prev => {
      ...
      const templates = exists
        ...
        : [...prev.formTemplates, { ...updated, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }];
  ...
  const duplicateFormTemplate = useCallback((id: string) => {
    setState(prev => {
      ...
      const copy: FormTemplate = {
        ...
        fields: src.fields.map(f => ({ ...f, id: `field_${Math.random().toString(36).slice(2)}` })),
      };
