...
export type AgentContext = 'dashboard' | 'projects' | 'templates' | 'automations' | 'contracts' | 'onboarding';
...
export const AI_MODELS: Record<AgentProvider, AIModel[]> = {
  claude: [
    { id: 'claude-sonnet-4-6', label: 'Claude Sonnet 4.6', description: 'Recomendado — equilíbrio entre custo e qualidade', isDefault: true },
    { id: 'claude-opus-4-7', label: 'Claude Opus 4.7', description: 'Mais poderoso — ideal para análises complexas' },
    { id: 'claude-haiku-4-5-20251001', label: 'Claude Haiku 4.5', description: 'Mais rápido e econômico' },
  ],
  gpt: [
    { id: 'gpt-4o', label: 'GPT-4o', description: 'Recomendado — multimodal e preciso', isDefault: true },
  ...
  gemini: [
    { id: 'gemini-2.5-pro', label: 'Gemini 2.5 Pro', description: 'Mais poderoso — lógica e contexto longo', isDefault: true },
    { id: 'gemini-2.5-flash', label: 'Gemini 2.5 Flash', description: 'Rápido e eficiente da série 2.5' },
    { id: 'gemini-3-flash', label: 'Gemini 3 Flash', description: 'Focado em velocidade e tarefas diárias' },
    { id: 'gemini-3.1-pro', label: 'Gemini 3.1 Pro', description: 'Lógica avançada e código — série 3.1' },
    { id: 'gemini-3.5-flash', label: 'Gemini 3.5 Flash', description: 'Mais recente — ideal para agentes e automações' },
  ],
};
...
export const agentService = {
  ...
  async generateInsights(context: AgentContext): Promise<{ insights: AgentInsight[]; provider: AgentProvider; logs: string[] }> {
  ...
  async executeApprovedAction(actionId: string): Promise<{ success: boolean; result: Record<string, unknown>; logs: string[] }> {
  ...
  async updateProviderConfig(provider: AgentProvider, apiKey?: string, model?: string): Promise<void> {
    ...
    const { data, error } = await supabase.functions.invoke('agency-settings', {
      body: { action: 'update_ai_provider', provider, apiKey: apiKey ?? null, model: model ?? null },
    });
